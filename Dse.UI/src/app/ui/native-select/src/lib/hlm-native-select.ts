...
export class HlmNativeSelect implements ControlValueAccessor {
  ...
  readonly selectId = input<string>(`hlm-native-select-${HlmNativeSelect._id++}`);
  ...
  readonly selectIconClass = input<ClassValue>('');
  ...
  readonly valueInput = input<string | undefined | null>('', {alias: 'value'});
  readonly value = linkedSignal(this.valueInput);

  readonly valueChange = output<string | undefined | null>();

...
@Component({
  ...
  template: `
    <div class="inline-flex flex-col space-y-4">
      ...
      <table brnCalendarGrid class="w-full border-collapse space-y-1">
        ...
        <tbody role="rowgroup">
          <tr *brnCalendarWeek="let week" class="mt-2 flex w-full">
            @for (date of week; track _dateAdapter.getTime(date)) {
              ...
              >
                <button brnCalendarCellButton [class]="_btnClass" [date]="date">
...
export class HlmCalendarRange<T> {
  ...
  constructor() {
    classes(
      () =>
        'group/calendar bg-background block p-3 [--cell-radius:var(--radius-md)] [--cell-size:--spacing(8)] in-data-[slot=card-content]:bg-transparent in-data-[slot=popover-content]:bg-transparent',
    );
  }
}

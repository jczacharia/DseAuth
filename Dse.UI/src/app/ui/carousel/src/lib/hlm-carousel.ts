...
export class HlmCarousel {
  ...
  readonly options: InputSignal<Omit<EmblaOptionsType, 'axis'> | undefined> = input<Omit<EmblaOptionsType, 'axis'>>();
  ...
  readonly canScrollPrev = this._canScrollPrev.asReadonly();
  private readonly _canScrollNext = signal(false);
  readonly canScrollNext = this._canScrollNext.asReadonly();

  private readonly _currentSlide = signal(0);
  readonly currentSlide = this._currentSlide.asReadonly();
  private readonly _slideCount = signal(0);
  readonly slideCount = this._slideCount.asReadonly();
